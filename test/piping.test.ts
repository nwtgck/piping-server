...
describe("piping.Server", () => {
  ...
  beforeEach(async () => {
    ...
    pipingServer = http.createServer(new piping.Server(false, logger).generateHandler(false));
    // Listen on the port
  ...
  it("should handle connection over HTTP/2 (receiver O, sender: O)", async () => {
    ...
    const http2PipingServer = http2.createServer(new piping.Server(false, logger).generateHandler(false));
